    ...
    public class NpcEntries
    {
        ...
        public void Interact(Interaction_Type l = Interaction_Type.none, byte answer = 0, byte slot = 0, byte ammt = 0)
        {
            ...
            switch (l)
            {
                case Interaction_Type.Talking:
                    {
                        LoadDialog(events, 1); 
                        globals.packet.character.NpcTalk(true);
                    }break;
        ...
        void LoadDialog(EventsinMapEntries events, byte unk)
        {
            ...
            for (int a = 0; a < events.SubEntry[unk].SubEntry.Count; a++)
            {
                    ...
                    case 1:
                        {
                            ...
                            {
                                location.WarpRequest( WarpType.Questwarp,globals.packet.character,events.SubEntry[unk].SubEntry[a].dialog2,  null);
                                Add = false;
                            }
                        } break;
                    case 2:
                ...
                if (Add)
                {
                    talk.SetSize(); globals.packet.character.DatatoSend.Enqueue(talk);
                }
            }
        }
    }
