...
namespace PServer_v2.DataLoaders
{
    ...
    public class Npc
    {
        ...
        public void Load(byte[] data, int ptr)
        {
            ...
            Element = byteXor(data[ptr]); ptr++;
            SkillID1 = wordXor(getWord(data, ptr)); ptr += 2;
            SkillID2 = wordXor(getWord(data, ptr)); ptr += 2;
            SkillID3 = wordXor(getWord(data, ptr)); ptr += 2;
    ...
    public class NpcDat
    {
        ...
        public NpcDat(cGlobals g)
        {
            globals = g;
        }
        public bool LoadNpc(string path)
        {
            globals.Log("loading Npc dat.....");
            try
            {
                NpcList.Clear();
