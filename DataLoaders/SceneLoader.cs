...
namespace PServer_v2.DataLoaders
{
    ...
    public class SceneDataFile
    {
        ...
        private static readonly object m_Lock = new object();
         ...
         public SceneDataFile(cGlobals g)
        {
            globals = g;
        }
        ...
        {
            globals.Log("loading Scence data.....");
            try
            {
                ...
                globals.Log(gz.ToString());
                globals.Log("failed"); return false; }
        }

        T WritetoFile<T>(T src)
        {
            StreamWriter e = null ;
            int max = src.GetType().GetFields().Where(f => f.IsPublic).Count();
            int count = 0;
            foreach (var f in src.GetType().GetFields().Where(f => f.IsPublic))
            {
                if (f.Name == "SceneID")
                {
                    File.Create("ScenceData//" + f.GetValue(src) + ".txt").Close();
                    e = new StreamWriter("ScenceData//"+f.GetValue(src)+".txt",true);
                }
                if ((char)f.Name[f.Name.Count() - 2] == "[".ToCharArray()[0])
                {
                    e.WriteLine(f.Name + "  :------->  " + string.Join(",",f.GetValue(src))); 
                }
                else
                    e.WriteLine(f.Name + "  :------->  " + f.GetValue(src));
        ...
        public SceneInfo GetSceneByID(UInt16 SceneID)
        {
            ...
            if (!bFound) { throw (new SceneNotFoundException("Scene => " + SceneID.ToString() + " could not be found.")); }
        ...
        public SceneInfo GetSceneByName(string SceneName)
        {
            ...
            if (!bFound) { throw (new SceneNotFoundException("Scene => \"" + SceneName + "\" could not be found.")); }
